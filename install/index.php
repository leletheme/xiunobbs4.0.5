...
// 安装初始化检测,放这里
$installed_conf = is_file(APP_PATH.'conf/conf.php') ? include APP_PATH.'conf/conf.php' : array();
if(!empty($installed_conf['installed'])) message(0, jump(lang('installed_tips'), '../'));
...
if(empty($action)) {
	...
	if($method == 'GET') {
		...
		$myisam_support = $mysql_support || $pdo_mysql_support;
		$innodb_support = $mysql_support || $pdo_mysql_support;
		...
		!in_array($type, array('mysql', 'pdo_mysql')) AND message('type', lang('db_type').' error');
		!in_array($engine, array('myisam', 'innodb')) AND message('engine', lang('db_engine').' error');
		!preg_match('/^[a-zA-Z0-9_]+$/', $name) AND message('name', lang('dbname_is_empty'));
		...
		if($r === FALSE) {
			if($errno == 1049) {
				if($type == 'mysql') {
					mysql_query("CREATE DATABASE `$name` DEFAULT CHARACTER SET utf8");
					$r = db_connect($db);
					...
					try {
						...
						$r = $link->exec("CREATE DATABASE `$name` DEFAULT CHARACTER SET utf8");
						if($r === FALSE) {
		...
		$create_ip = isset($longip) ? $longip : ip2long(ip());
		$update = array('username'=>$adminuser, 'email'=>$adminemail, 'password'=>$password, 'salt'=>$salt, 'create_date'=>$time, 'create_ip'=>$create_ip);
