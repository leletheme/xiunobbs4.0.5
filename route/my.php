...
$active = $action;
$profile_threadlist = mythread_find_by_uid($uid, 1, 7);
...
if(empty($action)) {
	...
	$credit_progress = group_credit_progress($user);
...
*/
	...
	$page = param(2, 1);
	$pagesize = 20;
	$totalnum = credit_log_count_by_uid($uid);
	$pagination = pagination(url('my-credit-{page}'), $totalnum, $page, $pagesize);
	$credit_log_list = credit_log_find_by_uid($uid, $page, $pagesize);
	$credit_log_types = credit_log_types();
	$header['title'] = '积分流水';
	$header['mobile_title'] = '积分流水';
	include _include(APP_PATH.'view/htm/my_credit.htm');
