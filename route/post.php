...
if($action == 'create') {
	...
	if($method == 'GET') {
		...
		!$pid AND message(-1, $errstr ? $errstr : lang('create_post_failed'));
		...
		$postlist = array($post);
		if(!empty($GLOBALS['xn_ai_last_reply_pid'])) {
			$ai_post = post_read(intval($GLOBALS['xn_ai_last_reply_pid']));
			if(!empty($ai_post) && intval($ai_post['tid']) == $tid) {
				$ai_post['floor'] = $thread['posts'] + 3;
				$postlist[] = $ai_post;
			}
		}
	...
	if($method == 'GET') {
		...
		if($isfirst) {
			...
			if(thread_brief_supported()) {
				xn_strlen($brief) > 240 AND message('brief', '摘要长度不能超过 240 个字符');
				$brief != array_value($thread, 'brief', '') AND $arr['brief'] = $brief;
			}
			if(thread_tags_supported() && $tags != array_value($thread, 'tags', '')) {
				$arr['tags'] = $tags;
			}
