...
if(empty($action) || $action == 'list') {
	...
	if($method == 'GET') {
		...
		$input['password'] = form_password('password', '');
		$input['credits'] = form_text('credits', intval($_user['credits']));
		$input['golds'] = form_text('golds', intval($_user['golds']));
		$input['rmbs'] = form_text('rmbs', intval($_user['rmbs']));
		$grouparr = arrlist_key_values($grouplist, 'gid', 'name');
		...
		$password = param('password');
		$_gid = param('_gid');
		$credits = intval(param('credits', 0));
		$golds = intval(param('golds', 0));
		$rmbs = intval(param('rmbs', 0));
		...
		$_new_user = user_read($_uid);
		foreach(array('credits'=>'积分', 'golds'=>'金币', 'rmbs'=>'人民币') as $_type=>$_name) {
			if(isset($update[$_type])) {
				$_change = intval($_new_user[$_type]) - intval($old[$_type]);
				function_exists('credit_log_create') AND credit_log_create($_uid, $_type, $_change, intval($_new_user[$_type]), '后台编辑用户'.$_name, 'admin_user_update', $_uid, $uid);
			}
		}
