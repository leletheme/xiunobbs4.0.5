...
function post_create($arr, $fid, $gid) {
	...
	if(!$pid && $pid !== FALSE) {
		$lastpost = db_find_one('post', array('tid'=>$arr['tid'], 'uid'=>$arr['uid'], 'create_date'=>$arr['create_date'], 'userip'=>$arr['userip'], 'isfirst'=>intval($arr['isfirst'])), array('pid'=>-1), array('pid'));
		$lastpost AND $pid = intval($lastpost['pid']);
	}
	if(!$pid) return $pid;
...
function post_message_fmt(&$arr, $gid) {
	...
	$arr['doctype'] == 2 && $arr['message_fmt'] = post_markdown_to_html($arr['message']);
...
function mythread_find_by_uid($uid, $page = 1, $pagesize = 20, $orderby = array('tid'=>-1)) {
	...
	$threadlist = thread_find(array('tid'=>$tids), $orderby, 1, $pagesize);
